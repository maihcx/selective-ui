...
! function(_plugin, e) {
    ...
    function SuperSelector(options) {
        let $this = {};
        if (superSelectStored) {
            return superSelectStored;
        }
        else {
            superSelectStored = $this;

            HTMLSelectElement.prototype.superSelect = function() {
        ...
        $this.CONFIG = {
            ...
            disabled: false,
            readonly: false,
            panelClass: 'superSelector-main-render',
            selectall: true,
        ...
        $this.FORM = {
            DATA: {
                ...
                systemIncrement: function(){
                    ...
                    MAIN_DIV.id = 'super_selector_main';
                    ...
                    $this.FORM.DATA.mutationObserver = new MutationObserver(function(mutations) {
                        mutations.forEach(function(mutation) {
                            if (mutation.attributeName != 'data-position' && mutation.attributeName != 'data-key' && mutation.attributeName != 'style' && mutation.attributeName != 'data-supershow') {
                                if (mutation.target.classList && !mutation.target.classList.contains('superSelector-selected-panel') && 
                                    !mutation.target.classList.contains('superSelector-placeholder') && 
                                    !mutation.target.classList.contains('super-selector-layout')) {
                                    // added node
                                    if (mutation.addedNodes.length > 0) {
                                        mutation.addedNodes.forEach(nodeExcuted => {
                                            ...
                                            switch (nodeExcuted.nodeName) {
                                                case 'SELECT':
                                                    if (!nodeExcuted.classList.contains('superSelector-INIT')) {
                    ...
                    const DIV_LAYOUT = LIB.nodeCreator({node: 'div', classList: 'super-selector-layout', id: 'layout_' + $this.FORM.DATA.keys[executionMark].secretKey}),
                    ...
                    let super_selector_main = document.getElementById('super_selector_main');
                    if (super_selector_main) {
                        super_selector_main.appendChild(DIV_LAYOUT);
                    ...
                    LIB.bindEvents(MAIN_PANEL, {
                        ...
                        click: function(event) {
                            ...
                            if (SELECT_ELEMENT.multiple) {
                                if (GROUP_ITEM) {
                                    if (GROUP_ITEM.classList.contains('empty-item') || GROUP_ITEM.classList.contains('superSelector-center-absolute') || GROUP_ITEM.classList.contains('group-item-searching')) return false;
        ...
        $this.SYSTEM_METHOD = {
            EXECUTER: function(styleMark, _options){
                        ...
                        selectElements.forEach(selectElement => {
                            if (selectElement.tagName === 'SELECT' && (!selectElement.dataset || LIB.isNullOrEmpty(selectElement.dataset.memKey))) {
                                ...
                                localSelectObj.overlay.event.on('mousedown', function(event) {
                                    if (configs.readonly || configs.disabled) return false;
                                    let EVENT_PATH = LIB.getPathOfEvent(event)[0];
                                    if (EVENT_PATH.classList.contains('superSelector-placeholder')) {
                                        localSelectObj.controls.open(event);
                                    }
                                    else if (EVENT_PATH.classList.contains('superSelector-deselect')) {
                ...
                let selectElement = selectObj.select.element,
                    selected_panel_classet = ['superSelector-MAIN'],
                    result = {};
                ...
                const SELECT_FRAME = LIB.nodeCreator({node: 'div', classList: selected_panel_classet}),
                      ...
                      SELECT_OVERLAY = LIB.nodeCreator({node: 'div', classList: ['superSelector-placeholder', 'flex-box', 'align-center'], tabIndex: 0, dataset: {panelkey: memKey}, style: {width: '100%', maxWidth: '100%', height: SELECT_HEIGHT, left: '0px'}}),
                      SELECT_OVERLAY_CONTENT = LIB.nodeCreator({node: 'span', classList: 'superSelector-placeholder-content', textContent: configs.placeholder}),
                      SELECT_OVERLAY_SYMBOL = LIB.nodeCreator({node: 'span', classList: 'superSelector-placeholder-symbol'})
                ;
                ...
                selectElement.classList.add('superSelector-INIT');
                ...
                if (selectElement.multiple) {
                    let selectedPanel = LIB.nodeCreator({
                        node: 'div', classList: ['superSelector-selected-panel', 'superSelector-seleted-panel', 'flex-box'], 
                    ...
                    selectedPanelProcess = {
                        ...
                        refresh: function() {
                            ...
                            selectOptions.forEach(selectOption => {
                                ...
                                const DIV_ELEMENT = LIB.nodeCreator({node: 'div', classList: ['superSelector-selected-item', 'flex-box', 'align-center']}),
                                      OPTION_MASK_TEXT = (selectOption.dataset?.mask) ? selectOption.dataset.mask : LIB.tagTranslate(selectOption.textContent),
                                      SPAN_VIEW_ELEMENT = LIB.nodeCreator({node: 'span', classList: 'superSelector-textshow', innerHTML: viewMask}),
                                      SPAN_CLEAR_ELEMENT = LIB.nodeCreator({node: 'span', classList: 'superSelector-deselect', dataset: {unValue: selectOption.value, memKey: memKey}})
                                ;
                ...
                let selectPanel = obj_select.panel = LIB.nodeCreator({node: 'div', classList: [$this.CONFIG.panelClass, 'flex-box', 'align-stretch', 'column'], dataset: {key: key}, style: {left: sizing.sizeRect.left + 'px', right: sizing.sizeRect.right + 'px', width: sizing.sizeRect.width + 'px', opacity: 0}}),
                    searchOverlay = LIB.nodeCreator({node: 'div', classList: ['superSelector-search', 'flex-box', 'align-center'], dataset: {key: key}, style: {left: '0px', height: sizing.sizeRect.height + 'px', minHeight: sizing.sizeRect.height + 'px', ...search_panel_cusstyle}}),
                    searchInput = LIB.nodeCreator({node: 'input', type: 'text', id: key+'_SEARCH', classList: 'superSelector-search-input'}),
                    loadingView = LIB.nodeCreator({node: 'span', classList: 'superSelector-center-absolute', textContent: configs.textloading}),
                ...
                if (LIB.isMobile) {
                    ...
                    panelMobileContent = LIB.nodeCreator({node: 'div', classList: ['superSelector-mobile-content-overlay', 'flex-box', 'column']});
                    panelMobileOverlay = LIB.nodeCreator({node: 'div', classList: ['superSelector-mobile-overlay', 'flex-box', 'align-center', 'justify-center']});
                ...
                if (selectPanel?.classList.contains('closing')){
                    ...
                    if (selectPanelMobile) {
                        ...
                        // LIB.setStyle(panel_target.querySelector('.superSelector-mobile-content-overlay'), {overflow: 'hidden', transition: null, transform: 'scale(0.7)'});
                    }
                    ...
                    delete selectElement.dataset.supershow;
                ...
                if (select_element.length > 0) {
                    if (configs.multiple && configs.selectall == true && (!selectObj.ajax.isDynamic || selectObj.ajax.pageNum == 1 || !selectObj.ajax.isPagination)) {
                        let optionGroupOther = LIB.nodeCreator({node: 'div', classList: 'group-other'}),
                            optionSelectAll = LIB.nodeCreator({node: 'a', classList: ['superSelector-a-controls', 'dis-left'], textContent: 'Chọn tất cả', dataset: {action: 'selectall'}}),
                            optionDeSelectAll = LIB.nodeCreator({node: 'a', classList: ['superSelector-a-controls', 'dis-right'], textContent: 'Bỏ chọn', dataset: {action: 'deselectall'}})
                        ;
                ...
                // empty panel
                    ...
                    const EMPTY_VIEW = LIB.nodeCreator({node: 'span', classList: 'superSelector-center-absolute', textContent: onLoading ? configs.textloading : configs.textnodata}),
                ...
                e_selectElement.classList.remove('superSelector-INIT');
        ...
        $this.destroy = async function(target) {
            const _await_promise = new Promise((resole, reject) => {
                for (var _OBJ_KEY in $this.FORM.DATA.keys) {
                    ...
                    document.querySelectorAll(excution+'.superSelector-INIT').forEach(selectElement => {
                        if (!selectElement.classList.contains('superSelector-INIT')) return true;
                ...
                if (Object.keys($this.FORM.DATA.keys).length == 0) {
                    ...
                    $this.FORM.DATA.keys = {};
                    document.getElementById('super_selector_main')?.remove();
    ...
    _plugin.SuperSelector = function(e = null) {
        return SuperSelector(e);
    }
}));